...
class Grader:
    ...
    def grade_part(self, part):
        ...
        print('----- END PART %s [took %s (max allowed %s seconds)' % (
            part.number, end_time - start_time, part.max_seconds))
        print()
    ...
    def assign_full_credit(self):
        if self.currentPart != None and not self.currentPart.failed:
