...
class MinimaxPlayer(Player):
    ...
    def __init__(self,
        ...
        self.plies = plies
    def get_best(self, board: GameBoard, depth: int, alpha: float, beta: float) -> List[int, Location]:
            if depth == 0 or board.is_terminal():
                return self.heuristic(board), None
            player = board.get_active_player()
            ans = float('-inf') if player == MAX_PLAYER else float('inf') # starts with the worst possible answer for the player
            best_move = None
            for move in board.get_legal_moves():
                new_board = board.make_move(move)
                here, next_move = self.get_best(new_board, depth-1, alpha, beta)
    ...
    def choose_move(self, board: GameBoard) -> Optional[Location]:
        ...
        ans, best_move = self.get_best(board, self.plies,float('-inf'),float('inf'))
        return best_move